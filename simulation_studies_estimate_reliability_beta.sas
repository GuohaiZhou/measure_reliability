...
proc import datafile="&path\under_beta_binomial_model\simu_&simu_iter..csv" out=temp dbms=csv replace; run;
data dat&nicut;
set temp;
if _N_<=&nicut;
run;
...
data dat&nicut;
set dat&nicut;
if &nicut=10 then fake_id = fake_provider_id_10;
if &nicut=100 then fake_id = fake_provider_id_100;
if &nicut=1000 then fake_id = fake_provider_id_1000;
run;
...
run;
proc export outfile="&path\under_beta_binomial_model\simu_&simu_iter._out_n_&nicut..csv" data=dat&nicut 
 ...
 * %est_reliability(nicut=10,start_simu=1,end_simu=1000);

 * %est_reliability(nicut=100,start_simu=1,end_simu=1000);

 %est_reliability(nicut=1000,start_simu=501,end_simu=1000);
