...
proc import datafile="&path\under_HGLM\simu_&simu_iter..csv" out=temp dbms=csv replace; run;
data dat&nicut;
set temp;
if _N_<=&nicut;
run;
...
run;
proc export outfile="&path\under_HGLM\simu_&simu_iter._out_n_&nicut..csv" data=dat&nicut 
